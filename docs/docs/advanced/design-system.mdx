...
ProtoTest has three surfaces people read: the [ProtoTrace viewer](./prototrace.md), the
[HTML report](./reporting.md) and these docs. They share **one** stylesheet of design tokens,
...
them, so a phase, an outcome or an entry kind looks the same everywhere.
...
If you write your own [report sink](./reporting.md) or render trace data somewhere else, start from the
