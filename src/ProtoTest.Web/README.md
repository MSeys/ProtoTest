...
Register more than one named backend (e.g. `name: "Admin"` and `name: "Customer"`) to drive several isolated, lazily started sessions from one test via `Proto.Context.Web("Admin")` / `Proto.Context.Web("Customer")`. Chain several actions on one component with `Flow()`, or log a named session in during setup with `[LoginAs("Administrator", session: "Admin")]` against an application-owned `IWebLoginStrategy`. See [docs/integrations/web.md](../../docs/integrations/web.md) for details.
