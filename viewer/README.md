...
The empty state can open the bundled `public/demos/prototest-demo.prototrace`. It is a real, complete parallel run of the unified SaaS demo: successful scenarios, partial diagnostic scenarios, two intentional failures - a REST shape mismatch and a web plan mismatch with the retained Playwright trace and its failure artifacts - and the run's own gate verdict. The number of scenarios grows with the demo, so the regeneration script checks a floor and the two named intentional failures rather than exact totals. It uses the same archive reader as a user-selected trace.

The bundled trace is a full demo run; a PostgreSQL container can enrich it with `ProtoTest:Database=postgres` (or `ProtoTest__Database=postgres`) when a container runtime is available.
