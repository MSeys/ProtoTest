...
public sealed class WebWaitContext
{
    ...
    public ValueTask<bool> EvaluateBooleanAsync(string script, CancellationToken cancellationToken = default)
        => _backend is IWebBackendJavaScript javascript
            ? javascript.EvaluateBooleanAsync(script, cancellationToken)
            : throw new WebBackendCapabilityException(
                $"Web backend '{_backend.Name}' does not support JavaScript evaluation.");
}
