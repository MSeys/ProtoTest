...
public sealed class GraphQLSchemaCoverageCollector : ProtoCoverageCollector
{
    ...
    public GraphQLSchemaCoverageCollector(string targetName, IConfiguration configuration) : base(targetName)
    {
        ArgumentNullException.ThrowIfNull(configuration);
        var clientConfiguration = configuration.GetSection($"ProtoTest:Clients:{targetName}");
        var source = clientConfiguration["GraphQL:Schema"];
        if (string.IsNullOrWhiteSpace(source))
            throw new InvalidOperationException(
                $"No GraphQL schema configured for target '{targetName}'. " +
                $"Set 'ProtoTest:Clients:{targetName}:GraphQL:Schema'.");
        _schema = GraphQLSchemaIndex.Parse(ProtoDocumentSource.LoadText(source, clientConfiguration["BaseUrl"]));
    }
