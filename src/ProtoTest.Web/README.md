...
Sessions are per-test: call `Proto.Context.Web("Admin")` / `Proto.Context.Web("Customer")` where the test needs them, and each isolated session is created lazily on first use (configure a name through `ProtoTest:Web:Sessions:{name}`). Chain several actions on one component with `Flow()`, or log a named session in during setup with `[LoginAs("Administrator", session: "Admin")]` against an application-owned `IWebLoginStrategy`. See [docs/integrations/web.md](../../docs/integrations/web.md) for details.
