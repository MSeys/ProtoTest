...
/// </summary>
internal sealed class NorthstarStore(
    TimeProvider time,
    NorthstarEventBus events,
    IDbContextFactory<NorthstarDbContext> contexts)
{
    ...
    public TenantResponse ProvisionTenant(string name, string planId, Uri apiBaseUrl)
    {
        ...
        lock (_gate)
        {
            ...
            if (db.Organizations.Any(candidate => candidate.Slug == slug))
            {
    ...
    public void DeleteTenant(string slug)
    {
        ...
        {
            using var db = contexts.CreateDbContext();
            var organization = db.Organizations.FirstOrDefault(candidate => candidate.Slug == slug)
    ...
    public (MembershipResponse Membership, string Token) CreateMemberForRole(string slug, string email, string role)
    {
        ...
        {
            using var db = contexts.CreateDbContext();
            var organization = RequireOrganization(db, slug);
    ...
    public NorthstarPrincipal Authenticate(string? secret)
    {
        ...
        {
            using var db = contexts.CreateDbContext();
            var organization = Aggregate(db).FirstOrDefault(candidate =>
                    candidate.Tokens.Any(token => token.Secret == secret))
                ?? throw NorthstarException.Unauthorized("The API token is not valid.");
            var token = organization.Tokens.First(candidate => candidate.Secret == secret);
            ...
            var member = organization.Members.First(candidate => candidate.Id == token.MemberId);
            db.SaveChanges();
    ...
    {
        return With(principal, (db, organization, actor) =>
        {
    ...
    public OrganizationResponse UpdateOrganization(NorthstarPrincipal principal, string? name)
    {
        return With(principal, (db, organization, actor) =>
        {
            ...
            Record(organization, "organization.updated", $"organization:{organization.Id}", actor.Email);
            return ToResponse(organization);
        });
    }
    ...
    public SubscriptionResponse ChangePlan(NorthstarPrincipal principal, string planId, int? seats)
    {
        ...
        {
            RequireScope(principal, TokenScopes.Admin);
            RequireRole(actor.Role, MemberRoles.CanManageBilling, "Only owners, administrators and billing contacts can change the plan.");
            Require(organization);
            ...
            Record(organization, "subscription.plan_changed", $"organization:{organization.Id}", actor.Email,
    ...
    public SubscriptionResponse CancelSubscription(NorthstarPrincipal principal)
    {
        return With(principal, (db, organization, actor) =>
        {
            ...
            RequireRole(actor.Role, MemberRoles.CanManageBilling, "Only owners, administrators and billing contacts can cancel the subscription.");
            Require(organization);
            organization.CancelAtPeriodEnd = true;
            Record(organization, "subscription.cancel_scheduled", $"organization:{organization.Id}", actor.Email);
            return ToSubscriptionResponse(organization);
        });
    }
    ...
    public SubscriptionResponse ResumeSubscription(NorthstarPrincipal principal)
    {
        ...
        {
            RequireScope(principal, TokenScopes.Billing);
            RequireRole(actor.Role, MemberRoles.CanManageBilling, "Only owners, administrators and billing contacts can resume the subscription.");
            organization.CancelAtPeriodEnd = false;
            Record(organization, "subscription.resumed", $"organization:{organization.Id}", actor.Email);
            return ToSubscriptionResponse(organization);
        });
    }
    ...
    public MembershipResponse InviteMember(NorthstarPrincipal principal, string email, string role)
    {
        ...
        {
            RequireScope(principal, TokenScopes.Admin);
            RequireRole(actor.Role, MemberRoles.CanManageMembers, "Only owners and administrators can invite members.");
            ValidateEmail(email);
            if (!MemberRoles.IsSupported(role))
            {
                throw NorthstarException.Validation($"Unsupported role '{role}'.");
            ...
            if (organization.Members.Any(member =>
            ...
            Record(organization, "member.invited", $"member:{member.Id}", actor.Email,
    ...
    public MembershipResponse UpdateMember(NorthstarPrincipal principal, string memberId, string role)
    {
        ...
        {
            RequireScope(principal, TokenScopes.Admin);
            RequireRole(actor.Role, MemberRoles.CanManageMembers, "Only owners and administrators can change member roles.");
            if (!MemberRoles.IsSupported(role))
            {
                throw NorthstarException.Validation($"Unsupported role '{role}'.");
            }
            ...
            member.Role = role;
            Record(organization, "member.role_changed", $"member:{member.Id}", actor.Email,
    ...
    {
        With(principal, (db, organization, actor) =>
        {
            RequireScope(principal, TokenScopes.Admin);
            RequireRole(actor.Role, MemberRoles.CanManageMembers, "Only owners and administrators can remove members.");
            var member = RequireMember(organization, memberId);
            ...
            member.Status = MemberStatuses.Removed;
            foreach (var token in organization.Tokens.Where(token => token.MemberId == member.Id).ToArray())
            {
                Revoke(organization, token);
            }

            Record(organization, "member.removed", $"member:{member.Id}", actor.Email);
        });
    }
    ...
    public CursorPage<ApiTokenResponse> ListTokens(NorthstarPrincipal principal, string? cursor, int limit)
    {
        return With(principal, (db, organization, actor) =>
        {
            var tokens = organization.Tokens
    ...
    public ApiTokenSecretResponse CreateToken(NorthstarPrincipal principal, string name, IReadOnlyList<string>? scopes)
    {
        ...
        {
            RequireScope(principal, TokenScopes.Admin);
            RequireRole(actor.Role, MemberRoles.CanManageMembers, "Only owners and administrators can create API tokens.");
            ArgumentException.ThrowIfNullOrWhiteSpace(name);
            ...
            var token = IssueToken(organization, actor, name.Trim(), requested, Now(organization));
            Record(organization, "token.created", $"token:{token.Id}", actor.Email);
    ...
    public void RevokeToken(NorthstarPrincipal principal, string tokenId)
    {
        ...
        {
            RequireScope(principal, TokenScopes.Admin);
            RequireRole(actor.Role, MemberRoles.CanManageMembers, "Only owners and administrators can revoke API tokens.");
            var token = organization.Tokens.FirstOrDefault(candidate => candidate.Id == tokenId)
                ?? throw NorthstarException.NotFound("token");
            Revoke(organization, token);
            Record(organization, "token.revoked", $"token:{token.Id}", actor.Email);
        });
    }
    ...
    public ProjectResponse CreateProject(NorthstarPrincipal principal, string name)
    {
        ...
        {
            RequireScope(principal, TokenScopes.Admin);
            RequireRole(actor.Role, MemberRoles.CanManageProjects, "Only owners and administrators can create projects.");
            Require(organization);
            ArgumentException.ThrowIfNullOrWhiteSpace(name);
            var active = organization.Projects.Count(project => project.Status == ProjectStatuses.Active);
    ...
    public ProjectResponse GetProject(NorthstarPrincipal principal, string projectId)
    {
        return With(principal, (db, organization, actor) =>
        {
            return ToResponse(RequireProject(organization, projectId));
        });
    }
    ...
    public ProjectResponse ArchiveProject(NorthstarPrincipal principal, string projectId)
    {
        ...
        {
            RequireScope(principal, TokenScopes.Admin);
            RequireRole(actor.Role, MemberRoles.CanManageProjects, "Only owners and administrators can archive projects.");
            ...
            Record(organization, "project.archived", $"project:{project.Id}", actor.Email);
            return ToResponse(project);
        });
    }
    ...
    public EnvironmentResponse CreateEnvironment(NorthstarPrincipal principal, string projectId, string name, string kind)
    {
        ...
        {
            RequireScope(principal, TokenScopes.Admin);
            RequireRole(actor.Role, MemberRoles.CanManageProjects, "Only owners and administrators can create environments.");
            Require(organization);
            ArgumentException.ThrowIfNullOrWhiteSpace(name);
            ...
            if (project.Environments.Any(environment =>
            ...
            Record(organization, "environment.created", $"environment:{environment.Id}", actor.Email,
    ...
    public DeploymentResponse Deploy(NorthstarPrincipal principal, string environmentId, string version, string commitSha)
    {
        return With(principal, (db, organization, actor) =>
        {
            ...
            var allowed = production
                ? MemberRoles.CanDeployToProduction(actor.Role)
                : MemberRoles.CanDeployToPreview(actor.Role);
            if (!allowed)
            {
            ...
            Record(organization, failed ? "deployment.failed" : "deployment.succeeded", $"deployment:{deployment.Id}", actor.Email,
        ...
        int limit)
    {
        return With(principal, (db, organization, actor) =>
        {
            var deployments = organization.Projects
    ...
    public DeploymentResponse GetDeployment(NorthstarPrincipal principal, string deploymentId)
    {
        return With(principal, (db, organization, actor) =>
        {
            return ToResponse(RequireDeployment(organization, deploymentId));
        });
    }
    ...
    public DeploymentResponse RollbackDeployment(NorthstarPrincipal principal, string deploymentId)
    {
        return With(principal, (db, organization, actor) =>
        {
            ...
            Record(organization, "deployment.rolled_back", $"deployment:{deployment.Id}", actor.Email,
    ...
    public InvoiceResponse PayInvoice(NorthstarPrincipal principal, long invoiceId, string method)
    {
        ...
        {
            RequireScope(principal, TokenScopes.Billing);
            RequireRole(actor.Role, MemberRoles.CanManageBilling, "Only owners, administrators and billing contacts can pay invoices.");
            Advance(organization);
            ...
            if (declined)
            {
                ...
                Record(organization, "invoice.payment_failed", $"invoice:{invoice.Id}", actor.Email,
    ...
    public InvoiceResponse VoidInvoice(NorthstarPrincipal principal, long invoiceId)
    {
        ...
        {
            RequireScope(principal, TokenScopes.Billing);
            RequireRole(actor.Role, MemberRoles.CanManageBilling, "Only owners, administrators and billing contacts can void invoices.");
            var invoice = RequireInvoice(organization, invoiceId);
            if (invoice.Status is InvoiceStatuses.Paid or InvoiceStatuses.Void)
            {
            ...
            Record(organization, "invoice.voided", $"invoice:{invoice.Id}", actor.Email);
    ...
    public CursorPage<WebhookEndpointResponse> ListWebhooks(NorthstarPrincipal principal, string? cursor, int limit)
    {
        return With(principal, (db, organization, actor) =>
        {
            var webhooks = organization.Webhooks
    ...
    public WebhookEndpointResponse CreateWebhook(NorthstarPrincipal principal, string url, IReadOnlyList<string>? events)
    {
        ...
        {
            RequireScope(principal, TokenScopes.Admin);
            RequireRole(actor.Role, MemberRoles.CanManageProjects, "Only owners and administrators can manage webhooks.");
            ...
            Record(organization, "webhook.created", $"webhook:{webhook.Id}", actor.Email);
    ...
    public void DeleteWebhook(NorthstarPrincipal principal, string webhookId)
    {
        With(principal, (db, organization, actor) =>
        {
            ...
            Record(organization, "webhook.removed", $"webhook:{webhook.Id}", actor.Email);
        });
    }
    ...
    public CursorPage<WebhookDeliveryResponse> ListWebhookDeliveries(NorthstarPrincipal principal, string? status, string? cursor, int limit)
    {
        return With(principal, (db, organization, actor) =>
        {
            var deliveries = organization.Deliveries
    ...
    public WebhookDeliveryResponse RedeliverWebhook(NorthstarPrincipal principal, string deliveryId)
    {
        ...
        {
            RequireScope(principal, TokenScopes.Admin);
            RequireRole(actor.Role, MemberRoles.CanManageProjects, "Only owners and administrators can redeliver webhooks.");
            ...
            Record(organization, "webhook.redelivered", $"webhook-delivery:{delivery.Id}", actor.Email);
    ...
    public void ReportDelivery(string organizationSlug, string deliveryId, bool success, string? error)
    {
        lock (_gate)
        {
            ...
            var organization = db.Organizations
                ...
                .FirstOrDefault(candidate => candidate.Slug == organizationSlug);
    ...
    public CursorPage<AuditEventResponse> ListAudit(NorthstarPrincipal principal, string? cursor, int limit)
    {
        return With(principal, (db, organization, actor) =>
        {
            RequireRole(actor.Role, MemberRoles.CanViewAudit, "This role cannot read the audit log.");
    ...
    private static void RequireRole(string role, Func<string, bool> predicate, string message)
    {
        if (!predicate(role))
        {
    ...
    {
        organization.Tokens.Remove(token);
    }
        ...
        IReadOnlyDictionary<string, string>? metadata = null)
    {
        organization.Audit.Add(new AuditEvent
        {
            ...
            Metadata = metadata is null
                ? []
                : [.. metadata.Select(pair => new AuditMetadataEntry(pair.Key, pair.Value))]
        });
    }
    ...
    /// <summary>Loads a whole organization aggregate, which is the consistency boundary every operation works on.</summary>
    private static IQueryable<Organization> Aggregate(NorthstarDbContext db)
        => db.Organizations
            ...
            .Include(entity => entity.Usage)
            .Include(entity => entity.Invoices)
                .ThenInclude(invoice => invoice.Payments)
            .Include(entity => entity.Webhooks)
            .Include(entity => entity.Deliveries)
            .Include(entity => entity.Audit)
            .AsSplitQuery();

    private static Organization RequireOrganization(NorthstarDbContext db, string slug)
        => Aggregate(db).FirstOrDefault(candidate => candidate.Slug == slug)
            ?? throw NorthstarException.NotFound("tenant");

    private static Organization RequireOrganizationById(NorthstarDbContext db, string organizationId)
        => Aggregate(db).FirstOrDefault(candidate => candidate.Id == organizationId)
            ?? throw NorthstarException.NotFound("tenant");

    /// <summary>
    /// Runs one operation against the caller's organization: loads the aggregate inside the store's
    /// lock, lets the operation mutate it, and persists once - so a write cannot be forgotten.
    /// </summary>
    private T With<T>(
        NorthstarPrincipal principal,
        Func<NorthstarDbContext, Organization, Membership, T> operation)
    {
        lock (_gate)
        {
            using var db = contexts.CreateDbContext();
            var organization = RequireOrganizationById(db, principal.Organization.Id);
            var actor = RequireMember(organization, principal.Member.Id);
            var result = operation(db, organization, actor);
            db.SaveChanges();
            return result;
        }
    }
    ...
    private void With(
        NorthstarPrincipal principal,
        Action<NorthstarDbContext, Organization, Membership> operation)
        => With(principal, (db, organization, actor) =>
        {
            operation(db, organization, actor);
            return true;
        });

    private static Membership RequireMember(Organization organization, string memberId)
        => organization.Members.FirstOrDefault(member =>
                member.Id == memberId && member.Status != MemberStatuses.Removed)
    ...
    private static Project RequireProject(Organization organization, string projectId)
        => organization.Projects.FirstOrDefault(project => project.Id == projectId)
    ...
    private static (Project Project, Environment Environment) RequireEnvironment(Organization organization, string environmentId)
    {
        ...
        {
            var environment = project.Environments.FirstOrDefault(candidate => candidate.Id == environmentId);
    ...
    private static OrganizationResponse ToResponse(Organization organization) => new(
        ...
        organization.Projects.Count(project => project.Status == ProjectStatuses.Active),
