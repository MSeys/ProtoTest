...
public sealed class ProtoTable
{
    ...
    public IReadOnlyList<ProtoTableRow> Rows
        => [.. Enumerable.Range(DataStartRow, RowCount).Select(row => new ProtoTableRow(this, row))];
    ...
    public ProtoColumn Column(params string[] headerPath)
    {
        ...
        return new ProtoColumn(this, ColumnNumber(headerPath));
    }
    ...
    public ProtoTableRow RowWhere(IReadOnlyList<string> keyHeaderPath, string value)
    {
        ...
        {
            if (string.Equals(_sheet.CellByNumber(row, column).Text, value, StringComparison.Ordinal))
            {
                return new ProtoTableRow(this, row);
            }
        }
    ...
    internal void RecordRead(int? row = null)
    {
        ...
        var identifier = row is null
            ...
            : $"{_sheet.Name}!A{row}:{lastColumn}";
        _context?.RecordObservation(new ProtoObservation("Sheets", "sheets.range", identifier));
    }
}
